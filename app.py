...
if st.session_state["role"] is None:
    ...
    with t2:
        u_reg = st.text_input("Tên mới"); p_reg = st.text_input("Mật khẩu mới", type="password")
    ...
    with t3:
        ua = st.text_input("Tài khoản Admin", key="ua"); pa = st.text_input("Mật khẩu Admin", type="password", key="pa")
...
# --- PHẦN NHẠC (SỬA LỖI TRỐNG SIDEBAR) ---
st.sidebar.divider()
    ...
    for i, row in f_df.iterrows():
        with st.expander(f"🏢 {row['Tên Công Ty']} - MST: {row['Mã Số Thuế']}"):
            ...
            with c1:
                ...
                st.markdown(f"🌍 [Bản đồ](https://google.com{maps_q})")
            ...
            if st.session_state["role"] == "admin":
                ...
                if st.session_state.get(f"edit_{i}"):
                    with st.form(f"f_e_{i}"):
                        en=st.text_input("Tên",row[0]); em=st.text_input("MST",row[1]); eo=st.text_input("Chủ",row[2]); ea=st.text_input("ĐC",row[3]); ep=st.text_input("SĐT",row[4])
